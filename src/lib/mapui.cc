...
void MapUI::PrintMenu() {
  ...
  case '2':
  {
    ...
    if (results.first != -1) {
      ...
      start = std::chrono::high_resolution_clock::now(); // test findclosestname
  ...
  case '4':
  {
    ...
    // auto results = map.TravellingTrojan_2opt(locations);
    auto stop = std::chrono::high_resolution_clock::now();
    auto duration = std::chrono::duration_cast<std::chrono::microseconds>(stop - start);
  ...
  case '6':
  {
    ...
    if (locations_filename == "") 
      location_names = {"Parking Center","Insomnia Cookies", "Expo Park/USC 2","Driveway","Arco","Ralphs", "KFC", "Chase","Trader Joes", "Chick-fil-A"};
    else
    ...
    if (dependencies_filename == "")
      dependencies = {{"Parking Center","Insomnia Cookies"},{"Insomnia Cookies", "Expo Park/USC 2"},{"Expo Park/USC 2","Driveway" },{"Driveway","Arco"},{"Arco","Ralphs"}, {"Ralphs","Chick-fil-A"}, {"Ralphs","Trader Joes"},{"Trader Joes", "Chick-fil-A"},{"Chick-fil-A", "Chase"}, {"Chase","KFC"}};
    else
