...
class TrojanMap {
 public:
  ...
  //----------------------------------------------------- User-defined functions
    ...
    std::string findMinIDButNotInVisited(std::map<std::string, double> &mindis, std::map<std::string, bool> &visited);
  
    // bool compare(std::pair<std::string, double> &p1, 
    //     std::pair<std::string, double> &p2);
    
    bool DeliveringTrojanHelper(std::string &loc,
            std::unordered_map<std::string,std::vector<std::string>> &neighbors,
            std::unordered_map<std::string,int> &states,
            std::vector<std::string> &result);

    bool hasCycle(std::string current_id, std::unordered_map<std::string, bool> &visited,
    std::unordered_map<std::string, std::string> &pred, std::vector<double> &square);
  
    void TravellingTrojan_Brute_force_aux(std::vector<std::vector<std::string>> &records,
                                                  std::vector<std::string> &location_ids, 
                                                  // double cur_distance, 
                                                  double &min_distance, 
                                                  std::vector<std::string> cur_path);

    void TravellingTrojanHelper(std::vector<std::vector<std::string>> &resP,
                                          std::vector<std::string> &location_ids,
                                          std::vector<std::vector<double>> &mapWei,
                                          std::unordered_set<int> &numvisited,double &resD, 
                                          std::vector<int> &curP, double curD);

    static bool CompareByDistance(std::pair<std::string, double> l1, 
                                            std::pair<std::string, double> l2);

    int CauculateEditDistance_recursion(std::string a, std::string b, int i, int j );
};
